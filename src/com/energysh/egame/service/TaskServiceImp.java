...
public class TaskServiceImp extends BaseService implements TaskService {
	...
	public void doIncome() {
		...
		this.getAppstoreDao()
				.excuteBySql(
						"UPDATE t_cpa_income t1 LEFT JOIN t_partners_income_conf t2 ON t1.user_id = t2.user_id SET t1.cpa_income = t1.active_num * IFNULL(t2.cpa_price, 0), t1.pub_status = 1");
	...
	public void uuQihooApp() {
		...
		int pageNo = 0;
		int pageSize = 10000;
		Long ss = (Long) this.getAppstoreDao().findObject(
				"select count(id) from TApp a where a.appSource=?",
				new Object[] { 1 });
		...
		for (int k = 1; k <= allPage; k++) {
			...
			List<Object> appList = this.getAppstoreDao().findListBySql(
					"select * from t_app a where a.app_source=?",
					new Object[] { 1 }, pageNo, pageSize, TApp.class);
			...
			for (Object obj : appList) {
				try {
					...
					String sign = Utils.getMD5Str(Utils.sortMap(map)
							+ MyConfigurer.getEvn("qhSecret"));
					map.put("sign", sign);
					try {
						Thread.sleep(1000);
					...
					String res = HttpUtils.sendGet(
							MyConfigurer.getEvn("qhMobiApiUrlApp"), map);

					JSONObject resObj = JSONObject.fromObject(res);
					JSONArray itemList = JSONArray.fromObject(resObj
							.get("items"));

					for (Object oo : itemList) {
						JSONObject mm = (JSONObject) oo;
						String pic1 = "", pic2 = "", pic3 = "", pic4 = "", pic5 = "";
						String[] url = mm.getString("screenshotsUrl")
								.split(",");
						...
						app.setVersionCode(Integer.valueOf(mm
						...
						app.setAppSize(Double.parseDouble(mm
								.getString("apkSize")));
	...
	public void hotSearch() {
		...
		try {
			...
			List<Map<String, Object>> list = this
					.getAppstoreDao()
					.findListMapBySql(
							"select key_name,count(*) as count from t_search_log where ctime BETWEEN ? and ? and key_name !='' GROUP BY key_name order by count desc LIMIT 10",
							new Object[] { startDate, endDate });
			// if(list.size() > 0)
			// this.getAppstoreDao().excuteBySql("delete from t_search_hot");
